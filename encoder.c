...
extern char direction_base;
extern char direction_elbow;
extern char direction_wrist;
extern char direction_gripper;`
...
void Encoder_4_Int_Handler(unsigned char state)
{
	...
	{
		// rising-edge interrupt
		if(direction_elbow == -1)
		{
			// backward
			Encoder_4_Count -= ENCODER_4_TICK_DELTA;
		}
		else if(direction_elbow == 1)
		{
			// forward
			Encoder_4_Count += ENCODER_4_TICK_DELTA;
		}
...
void Encoder_5_Int_Handler(unsigned char state)
{
	...
	{
		// rising-edge interrupt
		if(direction_wrist == -1)
		{
			// backward
			Encoder_5_Count -= ENCODER_5_TICK_DELTA;
		}
		else if(direction_wrist == 1)
		{
			// forward
			Encoder_5_Count += ENCODER_5_TICK_DELTA;
		}
...
void Encoder_6_Int_Handler(unsigned char state)
{
	...
	{
		// rising-edge interrupt
		if(direction_gripper == -1)
		{
			// backward
			Encoder_6_Count -= ENCODER_6_TICK_DELTA;
		}
		else if(direction_gripper == 1)
		{
			// forward
			Encoder_6_Count += ENCODER_6_TICK_DELTA;
		}
